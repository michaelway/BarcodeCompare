...
mergeBarcodeTable <- function(inFile1, inFile2) {
  if (is.null(inFile1))
    return(NULL)
  if (is.null(inFile2))
    return(NULL)
  ...
  #Deterime Plate Name
  plate1 <- unique(data1$V4)
  plate2 <- unique(data2$V4)  
  
  #Merge files
  bothPlates <- merge(data1[c(3,5)], data2[c(3,5)], by=c("V3"))
  
  bothPlates$conflicts <- bothPlates$V5.x %in% bothPlates$V5.y
...
plotBarcodeCompare <- function(merge) {
  par(xpd=TRUE)
...
summariseBarcodeCompare <- function(merge) {
